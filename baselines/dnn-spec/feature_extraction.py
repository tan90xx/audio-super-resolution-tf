...
class FeatureExtraction(object):
  ...
  def __init__(self, high_window_size=512,
    ...
    self.whiten_low = PCA(n_components=int(self.low_band_size), whiten=True)
    self.whiten_high = PCA(n_components=int(self.high_band_size), whiten=True)
    self.whiten_low_phase = PCA(n_components=int(self.low_band_size), whiten=True)
    self.whiten_high_phase = PCA(n_components=int(self.high_band_size), whiten=True)
  ...
  def stft(self, x, window_size, window_shift):
    ...
    X = sp.array([sp.fft.fft(w*x[i:i+window_size])
  ...
  def istft(self, X, n_samples, window_shift):
    ...
    for n,i in enumerate(range(0, len(x)-window_size, window_shift)):
        x[i:i+window_size] += sp.real(sp.fft.ifft(X[n]))
    return x
  ...
  def extract_low_high(self, X_log_magnitude, X_phase, split=True):
    ...
    X_log_magnitude = X_log_magnitude[:, :int((N/2)+1)]
    # Conjugate symmetric only take non-redundant points
    X_phase = X_phase[:, :int((N/2)+1)]

    # If we want to split into high and low components
    # I break out the cases manually because it's easier to follow than eqn
    if split:
      if self.upsample == 2:
        X_low, X_high = split(X_log_magnitude, int((N/4)+1))
        X_low_phase, X_high_phase = split(X_phase, int((N/4)+1))
      elif self.upsample == 4:
        X_low, X_high = split(X_log_magnitude, int((N/8)+1))
        X_low_phase, X_high_phase = split(X_phase, int((N/8)+1))
      elif self.upsample == 6:
        X_low, X_high = split(X_log_magnitude, int(np.ceil((N/12)+1)))
        X_low_phase, X_high_phase = split(X_phase, int(np.ceil((N/12)+1)))
      elif self.upsample == 8:
        X_low, X_high = split(X_log_magnitude, int((N/16)+1))
        X_low_phase, X_high_phase = split(X_phase, int((N/16)+1))
