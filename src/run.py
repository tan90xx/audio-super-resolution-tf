...
def make_parser():
  ...
  # ttyadd: proposed
  train_parser.add_argument('--model', default='proposed',
    choices=('proposed', 'audiounet', 'audiotfilm', 'dnn', 'spline'),
    help='model to train')
  # ttyadd : loss function
  train_parser.add_argument('--loss_func', default='L2',
    choices=('L2','MAE', 'MSE', 'F', 'RI', 'TF', 'RI_MAG', 'PCM', 'T_PCM'),
    help='loss to train')
  train_parser.add_argument('--train', default='../data/timit-train.2.2048.1024.h5',
    help='path to h5 archive of training patches')
  train_parser.add_argument('--val', default='../data/timit-val.2.2048.1024.h5',
  ...
  #train_parser.add_argument('--layers', default=8, type=int,
  ...
  #  help='learning rate')
  train_parser.add_argument('--r', type=int, default=2, help='upscaling factor')
  ...
  train_parser.add_argument('--pool_size', type=int, default=2, help='size of pooling window')
  train_parser.add_argument('--strides', type=int, default=2, help='pooling stide')
  ...
  eval_parser.add_argument('--out-label', default='proposed',
    help='append label to output samples')
  eval_parser.add_argument('--wav-file-list', default='../data/Corpus/test.txt',
  ...
  eval_parser.add_argument('--speaker', default='single', choices=('single', 'multi'),
    help='number of speakers being trained on')
  eval_parser.add_argument('--pool_size', type=int, default=2, help='size of pooling window')
  eval_parser.add_argument('--strides', type=int, default=2, help='pooling stide')
  eval_parser.add_argument('--patch_size', type=int, default=2048, help='Size of patches over which the model operates')
  return parser
...
def eval(args):
  # load model
  args.loss_func = args.logname.split('.')[-1]
    ...
    with open(args.wav_file_list) as f:
      ...
      for line in tqdm(f):
        try:
          #print((line.strip()))
          # ttyadd: for corpus
          upsample_wav('../data/Corpus/'+line.strip(), args, model)
...
def get_model(args, n_dim, r, from_ckpt=False, train=True, grocery='false'):
  ...
  # ttyadd: set opt for different models
  #if train:
  ...
  if args.model == 'proposed':
    model = models.Proposed(from_ckpt=from_ckpt, n_dim=n_dim, r=r, opt_params=opt_params, log_prefix=args.logname)
    ...
    model = models.AudioTfilm(from_ckpt=from_ckpt, n_dim=n_dim, r=r, opt_params=opt_params, log_prefix=args.logname,
                              pool_size = args.pool_size, strides=args.strides)
  elif args.model == 'dnn':
    model = models.DNN(from_ckpt=from_ckpt, n_dim=n_dim, r=r, opt_params=opt_params, log_prefix=args.logname)
  elif args.model == 'spline':
    model = models.Spline(from_ckpt=from_ckpt, n_dim=n_dim, r=r, opt_params=opt_params, log_prefix=args.logname)
  else:
