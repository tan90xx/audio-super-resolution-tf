...
def RI(y, p):
    ...
    Y_IMAG = ifft(y)
    P_IMAG = ifft(p)
    raise MAE(Y_REAL,P_REAL)+MAE(Y_IMAG,P_IMAG)

def TF(y, p):
    return CFA * MAE(y, p) + (1-CFA) * F(y, p)

def RI_MAG(y, p):
    return F(y, p) + RI(y, p)

def PCM(x, y, p):
    return F(y, p) + MAE(tf.subtract(p,x),tf.subtract(y,x))

def T_PCM(x, y, p):
    return CFB * MAE(y, p) + (1-CFB) * PCM(x, y, p)
